...
class Stream {
    ...
    static fromSSEResponse(response, controller) {
        ...
        async function* iterator() {
            if (consumed) {
                throw new error_1.LlamaAPIClientError('Cannot iterate over a consumed stream, use `.tee()` to split the stream.');
            }
            consumed = true;
            let done = false;
            try {
...
async function* _iterSSEMessages(response, controller) {
    if (!response.body) {
        ...
        if (typeof globalThis.navigator !== 'undefined' &&
            ...
            throw new error_1.LlamaAPIClientError(`The default react-native fetch implementation does not support streaming. Please use expo/fetch: https://docs.expo.dev/versions/latest/sdk/expo/#expofetch-api`);
        }
        throw new error_1.LlamaAPIClientError(`Attempted to iterate over a response with no body`);
    }
...
async function* iterSSEChunks(iterator) {
    ...
    for await (const chunk of iterator) {
        ...
        const binaryChunk = chunk instanceof ArrayBuffer ? new Uint8Array(chunk)
            : typeof chunk === 'string' ? new globalThis.TextEncoder().encode(chunk)
                : chunk;
